    ...
    try:
        ...
        # - 첫 매수
            ...
            if ma5 < ma20 and ma5_before3 > ma5_before2 and ma5_before2 < ma5 and getHasCoinCnt(balances) < MAXCOINCNT:
                ...
                # 실제 매수 금액
                tempBuyPrice = get_tick_size(avgPrice)

                # 1퍼센트 상승된 가격
                avgPrice *= 1.01 

                # 지정가 매도 변수
                tempSellPrice = get_tick_size(avgPrice)

                # 지정가매도
                sellCoinLimit(upbit, ticker, avgPrice, coinVolume)

                chart = get_coin_chart(ticker=ticker, 
                                        interval="minute15",
                                        rsi_interval="minute60",
                                        pos_before3=-4,
                                        pos_before2=-3,
                                        ...
                                        buy_pos=-2,
                                        sell_pos=-2,
                                        buy_price=tempBuyPrice,          # 실제 매수 체결가
                                        target_sell_price=tempSellPrice  # 지정가 매도 단가
                                       )
                send_gmail("✅ 매수 완료", message, chart)
