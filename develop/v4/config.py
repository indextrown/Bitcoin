...
from develop.v4.trade_logic import StrategyConfig


@dataclass(frozen=True)
class BacktestConfig:
    """실거래와 분리된 V4 백테스트 체결·실행 가정값입니다."""
...
@dataclass(frozen=True)
class V4Config:
    """V4 봇·백테스트·시각화가 공통으로 읽는 전체 설정입니다."""

    ticker: str = "KRW-ETH"  # 거래·분석할 업비트 마켓 티커입니다.
    interval: str = "minute240"  # RSI 전략이 기준으로 삼을 업비트 캔들 간격입니다.
    strategy: StrategyConfig = field(default_factory=StrategyConfig)  # V4 RSI 회복·청산 전략 설정입니다.
...
# 실제 봇은 ticker·interval·strategy만, 시뮬레이터는 backtest까지 함께 읽는 기본 V4 설정입니다.
V4_CONFIG = V4Config()
