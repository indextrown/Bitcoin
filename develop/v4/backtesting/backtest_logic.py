"""V4의 RSI 회복 진입·익절·손절 규칙을 과거 OHLCV에 적용하는 순수 백테스트입니다."""
...
from develop.v4.config import V4_CONFIG, V4Config, interval_to_timedelta
from develop.v4.trade_logic import build_signal, decide_trade
...
def _is_scheduled_execution_time(timestamp: pd.Timestamp, cron_duration: pd.Timedelta) -> bool:
    """자정 기준 V4 크론 주기에 맞는 판단 시점인지 반환합니다."""
...
def run_backtest(
    ...
    """V4 규칙으로 크론 실행·다음 원본 봉 시가 체결을 가정해 결과를 계산합니다.

    V3와 동일하게 크론 시점마다 부분 전략 봉으로 RSI를 만들고, 미래 데이터를 쓰지 않기
    위해 다음 원본 봉의 시가로 주문을 체결합니다. 차이는 V4가 RSI 회복 진입과 손절·익절
    규칙을 ``develop.v4.trade_logic.decide_trade``에서 사용한다는 점입니다.

    Args:
        ohlcv: 크론 실행 시점보다 같거나 짧은 간격의 시간순 원본 OHLCV 데이터입니다.
        config: 티커·V4 전략·백테스트 체결 가정을 담은 전체 설정입니다.
        strategy_candle_anchor: 업비트가 현재 전략 봉을 시작한 실제 시각입니다.
        simulation_start: 시작 원금을 적용하고 그래프에 표시할 포함 시각입니다.
        simulation_end: 그래프에 표시할 배타적 종료 시각입니다.

    Returns:
        RSI, 자산 곡선, 실제 체결 기록, 크론 판단 시각을 담은 백테스트 결과입니다.
